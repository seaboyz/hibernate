- [Hibernate learning notes](#hibernate-learning-notes)
      - [Add maven dependency](#add-maven-dependency)
      - [hibernate](#hibernate)
      - [postgresql](#postgresql)
      - [Steps without Hibernate](#steps-without-hibernate)
    - [The Hibernate way](#the-hibernate-way)
    - [3 Ways of Configure Hibernate](#3-ways-of-configure-hibernate)
      - [1. hibernate.cfg.xml](#1-hibernatecfgxml)
      - [2. hibernate.properties](#2-hibernateproperties)
    - [Add annotated class to hibernate](#add-annotated-class-to-hibernate)
    - [create a seesion factory session and user session to save model](#create-a-seesion-factory-session-and-user-session-to-save-model)
    - [`@Entity(name="table_name"` vs `@Table(name"table_name")`](#entitynametable_name-vs-tablenametable_name)
    - [create a singular name table](#create-a-singular-name-table)
    - [use column anotation to name db column](#use-column-anotation-to-name-db-column)
    - [Hibernate Testing](#hibernate-testing)
    - [When hiberate create a the connection](#when-hiberate-create-a-the-connection)
    - [When hiberate create table?](#when-hiberate-create-table)
      - [@Transient](#transient)
      - [Primary Key](#primary-key)
        - [@NatualId](#natualid)
        - [@Id](#id)
        - [@GeneratedValue(strategy = GenerationType.AUTO)](#generatedvaluestrategy--generationtypeauto)
        - [@Genericgenerator(name = "generator", strategy = "native")](#genericgeneratorname--generator-strategy--native)
        - [UUID](#uuid)
        - [interger Id](#interger-id)
      - [value object vs entity object](#value-object-vs-entity-object)
...
- [Can a pendding order be in the cart?](#can-a-pendding-order-be-in-the-cart)
      - [JPA (Java Persistence API) @Entity](#jpa-java-persistence-api-entity)
      - [@OneToMany](#onetomany)
      - [Add two addresses to one user](#add-two-addresses-to-one-user)
    - [Bidirectional @OneToMany](#bidirectional-onetomany)
      - [@OneToMany(mappedBy = "customer")](#onetomanymappedby--customer)
      - [@ManyToOne](#manytoone)
      - [@JoinColumn(name = "customer_id") - foreign key name in the address table](#joincolumnname--customer_id---foreign-key-name-in-the-address-table)
    - [Unidierctional @OneToMany](#unidierctional-onetomany)
    - [Unidierctional vs Bidirectional](#unidierctional-vs-bidirectional)
      - [Lazy vs Eager(customer side)](#lazy-vs-eagercustomer-side)
        - [fetch = FetchType.LAZY](#fetch--fetchtypelazy)
      - [fetch = FetchType.EAGER](#fetch--fetchtypeeager)
      - [Proxy](#proxy)
    - [@OneToOne](#onetoone)
      - [1. Unidirectional @OneToOne](#1-unidirectional-onetoone)
      - [2. Bidirectional @OneToOne](#2-bidirectional-onetoone)
    - [@ManyToMany](#manytomany)
      - [1. Unidirectional @ManyToMany](#1-unidirectional-manytomany)
      - [2. Bidirectional @ManyToMany](#2-bidirectional-manytomany)
      - [3. ManyToMany with @JoinTable with additional columns](#3-manytomany-with-jointable-with-additional-columns)
    - [Cascade types](#cascade-types)
      - [CascadeType.ALL](#cascadetypeall)
      - [@Cascade = CascadeType.PERSIST](#cascade--cascadetypepersist)
      - [@Cascade = CascadeType.REMOVE](#cascade--cascadetyperemove)
      - [@Cascade = CascadeType.MERGE](#cascade--cascadetypemerge)
      - [@Cascade = CascadeType.REFRESH](#cascade--cascadetyperefresh)
      - [@Cascade = CascadeType.DETACH](#cascade--cascadetypedetach)
      - [@Cascade = CascadeType.REPLICATE](#cascade--cascadetypereplicate)
      - [@Cascade = CascadeType.ALL](#cascade--cascadetypeall)
        - [mvn exec](#mvn-exec)
    - [Dao (Data Access Object)](#dao-data-access-object)
    - [Repository (Data Access Object)](#repository-data-access-object)
  ...
  - [DAO](#dao)
      - [CRUD](#crud)
      - [??? is it possible one object can be exist in multiple sessions???](#-is-it-possible-one-object-can-be-exist-in-multiple-sessions)
    - [every CRUD is a single transaction](#every-crud-is-a-single-transaction)
    - [CREATE](#create)
    - [READ](#read)
      - [persistent](#persistent)
      - [detached](#detached)
    - [UPDATE](#update)
      - [Customer UPDATE process(service layer)](#customer-update-processservice-layer)
      - [Customer UPDATE(Dao layer)](#customer-updatedao-layer)
    - [DELETE](#delete)
    - [save(), persist(), update(), merge(), saveOrUpdate()](#save-persist-update-merge-saveorupdate)
      - [session.persist()](#sessionpersist)
      - [session.save()](#sessionsave)
      - [session.merge()](#sessionmerge)
    ...
    - [Persistent ==> Detached](#persistent--detached)
    ...
    - [Add lombok (optional)](#add-lombok-optional)
...
* `SessionFactory` is a `factory` of `sessions`. However, `Session` is a `session` of `persistence`
* `SessionFactory` has to be a singleton.
